...
server.registerTool("scaffoldReactComponent", {
...
server.registerTool("addRoute", {
...
}, async ({ routePath, importPath, componentName }) => {
    const workspaceRoot = process.cwd();
    ...
    src = src.replace(/(<Route\s+path=\"\*\"\s+element=\{<NotFound\s*\/>\}\s*\/\>)/, `${routeLine}\n$1`);
...
server.registerTool("runBunScript", {
...
server.registerTool("modifyTailwindClasses", {
